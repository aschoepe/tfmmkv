...
fprintf(stderr, "zIn len %d\n", strlen(zIn));
  for(m=0; m<256; m++){ s[m] = m; }
  for(j=0, m=n=0; m<256; m++, n++){
fprintf(stderr, "j %d n %d m %d t %d s[m] %d zIn[n] %d\n", j, n, m, t, s[m], zIn[n]);
    j += s[m] + zIn[n];
    if( zIn[n]==0 ){ n = -1; }
    t = s[j];
    s[j] = s[m];
...
fprintf(stderr, "++\n");
  i = j = 0;
  for(n=0; n<N-2; n+=2){
fprintf(stderr, "j %d n %d t %d s[i] %d s[j] %d\n", j, n, t, s[i], s[j]);
    i++;
    t = s[i];
fprintf(stderr, "t %d j %d\n", t, j);
    j += t;
fprintf(stderr, "j %d\n", j);
    s[i] = s[j];
    s[j] = t;
...
fprintf(stderr, "FOSSIL_BUILD_EPOCH\n");
#define str(s) #s
